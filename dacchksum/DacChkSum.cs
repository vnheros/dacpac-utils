    ...
    internal class DacChkSum
    {
        ...
        public void Run()
        {
            ...
            using (var package = new DacPackage(inputFile))
            {
                ...
                if(!string.IsNullOrEmpty(_args.ReplaceModel)) //lvhung replace existing model = new model
                {
                    Console.Out.WriteLine("Replacing model");
                    package.ReplaceModel(_args.ReplaceModel);
                    save = true;
                    Console.Out.WriteLine("Done");
                }

                if (!string.IsNullOrEmpty(_args.ReplaceOrigin)) //lvhung replace existing origin = new origin
                {
                    Console.Out.WriteLine("Replacing origin");
                    package.ReplaceOrigin(_args.ReplaceOrigin);
                    save = true;
                    Console.Out.WriteLine("Done");
                }
